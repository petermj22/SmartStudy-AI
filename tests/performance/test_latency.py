...
class TestInferenceLatency:
    ...
    def test_feature_buffer_append_under_1ms(self):
        """Buffer append should be < 1ms."""
        from backend.ml.feature_buffer import FeatureBuffer

        buf = FeatureBuffer(max_size=900)
        features = {"avg_ear": 0.28, "mar": 0.3, "blink_rate": 15.0}
        ...
        avg_ms = np.mean(times)
        assert avg_ms < 1, f"Avg buffer append {avg_ms:.2f}ms exceeds 1ms"

    def test_buffer_stats_under_5ms(self):
        """Rolling statistics should be < 5ms."""
        from backend.ml.feature_buffer import FeatureBuffer

        buf = FeatureBuffer(max_size=900)
        for i in range(900):
            buf.add({"avg_ear": 0.28 + np.random.randn() * 0.02})

        times = []
        for _ in range(100):
            t0 = time.perf_counter()
            stats = buf.compute_stats("avg_ear", window=30)
            elapsed_ms = (time.perf_counter() - t0) * 1000
            times.append(elapsed_ms)

        avg_ms = np.mean(times)
        assert avg_ms < 5, f"Avg stats computation {avg_ms:.2f}ms exceeds 5ms"
